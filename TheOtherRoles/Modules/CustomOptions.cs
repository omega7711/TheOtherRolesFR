...
namespace TheOtherRoles {
    ...
    class GameOptionsMenuStartPatch {
        public static void Postfix(GameOptionsMenu __instance) {
            if (GameObject.Find("TORSettings") != null) { // Settings setup has already been performed, fixing the title of the tab and returning
                GameObject.Find("TORSettings").transform.FindChild("GameGroup").FindChild("Text").GetComponent<TMPro.TextMeshPro>().SetText("Les paramètres de The Other Roles");
                return;
            }
            if (GameObject.Find("ImpostorSettings") != null) {
                GameObject.Find("ImpostorSettings").transform.FindChild("GameGroup").FindChild("Text").GetComponent<TMPro.TextMeshPro>().SetText("Paramètres des rôles Imposteurs");
                return;
            }
            if (GameObject.Find("NeutralSettings") != null) {
                GameObject.Find("NeutralSettings").transform.FindChild("GameGroup").FindChild("Text").GetComponent<TMPro.TextMeshPro>().SetText("Paramètres de rôles Neutres");
                return;
            }
            if (GameObject.Find("CrewmateSettings") != null) {
                GameObject.Find("CrewmateSettings").transform.FindChild("GameGroup").FindChild("Text").GetComponent<TMPro.TextMeshPro>().SetText("Paramètres de rôles Crewmates");
                return;
            }
            if (GameObject.Find("ModifierSettings") != null) {
                GameObject.Find("ModifierSettings").transform.FindChild("GameGroup").FindChild("Text").GetComponent<TMPro.TextMeshPro>().SetText("Paramètres de modificateurs");
                return;
            }
    ...
    class GameOptionsDataPatch
    {
        ...
        private static void Postfix(ref string __result)
        {
            ...
            string hudString = counter != 0 ? Helpers.cs(DateTime.Now.Second % 2 == 0 ? Color.white : Color.red, "(Utilisez la molette de défilement si nécessaire)\n\n") : "";

            switch (counter) {
                case 0:
                    hudString += "Page 1: Paramètres Vanilla \n\n" + __result;
                    break;
                case 1:
                    hudString += "Page 2: Paramètres de The Other Roles \n" + buildOptionsOfType(CustomOption.CustomOptionType.General, false);
                    break;
                case 2:
                    hudString += "Page 3: Taux de rôle et de modificateur \n" + buildRoleOptions();
                    break;
                case 3:
                    hudString += "Page 4: Paramètres des rôles Imposteurs \n" + buildOptionsOfType(CustomOption.CustomOptionType.Impostor, false);
                    break;
                case 4:

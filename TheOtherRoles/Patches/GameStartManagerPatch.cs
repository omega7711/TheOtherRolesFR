            ...
            public static void Postfix(GameStartManager __instance) {
                ...
                foreach (InnerNet.ClientData client in AmongUsClient.Instance.allClients.ToArray()) {
                    ...
                    if (dummyComponent != null && dummyComponent.enabled)
                        ...
                        message += $"<color=#FF0000FF>{client.Character.Data.PlayerName} a une version différente ou n'a pas The Other Roles\n</color>";
                    } else {
                        ...
                        if (diff > 0) {
                            ...
                            message += $"<color=#FF0000FF>{client.Character.Data.PlayerName} a une version modifiée de TOR v{playerVersions[client.Id].version.ToString()} <size=30%>({PV.guid.ToString()})</size>\n</color>";
                            versionMismatch = true;
                        }
                    }
                }
                ...
                // Client update with handshake infos
                    ...
                    if (!playerVersions.ContainsKey(AmongUsClient.Instance.HostId) || TheOtherRolesPlugin.Version.CompareTo(playerVersions[AmongUsClient.Instance.HostId].version) != 0) {
                        ...
                        __instance.GameStartText.text = $"<color=#FF0000FF>L'hôte n'a pas ou a une version différente de The Other Roles\nVous serez kick dans {Math.Round(10 - kickingTimer)}s</color>";
                        __instance.GameStartText.transform.localPosition = __instance.StartButton.transform.localPosition + Vector3.up * 2;
                    } else if (versionMismatch) {
                        __instance.GameStartText.text = $"<color=#FF0000FF>Joueurs avec une version différente:\n</color>" + message;
                        __instance.GameStartText.transform.localPosition = __instance.StartButton.transform.localPosition + Vector3.up * 2;
                    } else {
                        __instance.GameStartText.transform.localPosition = __instance.StartButton.transform.localPosition;

        ...
        static void Postfix(PlayerControl __instance, [HarmonyArgument(0)]GameData.PlayerInfo target)
        {
            ...
            if (isMedicReport || isDetectiveReport)
            {
                ...
                if (deadPlayer != null && deadPlayer.killerIfExisting != null) {
                    ...
                    if (isMedicReport) {
                        msg = $"Report de cadavre: A été tué il y a {Math.Round(timeSinceDeath / 1000)}s!";
                    } else if (isDetectiveReport) {
                        if (timeSinceDeath < Detective.reportNameDuration * 1000) {
                            msg =  $"Report de cadavre: Le tueur semble être {deadPlayer.killerIfExisting.Data.PlayerName}!";
                        } else if (timeSinceDeath < Detective.reportColorDuration * 1000) {
                            var typeOfColor = Helpers.isLighterColor(deadPlayer.killerIfExisting.Data.DefaultOutfit.ColorId) ? "lighter" : "darker";
                            msg =  $"Report de cadavre: Le tueur semble être de couleur {typeOfColor} !";
                        } else {
                            msg = $"Report de cadavre: La cadavre est trop vieurx pour récupérer la moindre information!";
                        }
                    }

...
namespace TheOtherRoles {
    ...
    public static class Helpers
    {
        ...
        {
            if (roleInfo.name == "Chacal") 
            {
                var getSidekickText = Jackal.canCreateSidekick ? " et recrute un Acolyte" : "";
                return cs(roleInfo.color, $"{roleInfo.name}: Tue tout le monde{getSidekickText}");  
            }

            if (roleInfo.name == "Inversion") 
            {
